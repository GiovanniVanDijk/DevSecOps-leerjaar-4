...
from html import escape # toegevoegd voor cross-site scripting
...
def index():
    ...
    # oude code met vulnerability XSS:
    # return templates.main_page(quotes, request.user_id, request.args.get('error'))
    
    error_msg = request.args.get('error')
    safe_error = escape(error_msg) if error_msg else None
...
def get_comments_page(quote_id):
    # oude code met vulnerability SQL injectie:
    ...
    comments = db.execute(
        "select text, datetime(time,'localtime') as time, name as user_name from comments c left join users u on u.id=c.user_id where quote_id=? order by c.id",
        (quote_id,)
    ).fetchall()
